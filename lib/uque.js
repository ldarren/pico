...
function _increIn(mem, keyIn, config, cb){
  mem.gets(keyIn, function(err, result){
    var value = parseInt(result[keyIn]);
    console.log('_increIn: '+(value + 1 === config.size));
    if (value + 1 === config.size) value = 0;
    mem.cas(keyIn, value+1, result.cas, config.life, function(err, result){
      if (result) {cb(err, value); return;}
      _increIn(mem, keyIn, config, cb);
    });
  });
}
...
function _getPushRight(mem, uuid, config, cb){
  _increIn(mem, uuid+'in', config, function(err, result){
    if (err) 
  ...
  _getPushRight(mem, uuid, config, function(err, i){
    mem.set(uuid+i, value, config.life, function(err, result){
      ...
      self.length(uuid, function(err, len){
        console.log('push '+util.inspect(len));
        mem.increment(uuid+'tail', 1, function(err, result){
          if (len[0] >= config.size){
            mem.increment(uuid+'head', 1, cb);
          }
        });
      });
    });
...
uQue.prototype.length = function(uuid, cb){
  ...
  _getInt(mem, uuid+'tail', function(err, tail){
    _getInt(mem, uuid+'head', function(err, head){
      if (err) {cb(err, -1);}
      console.log('length tail['+tail+'] head['+head+'] len['+(tail-head)+'] size['+config.size+']');
